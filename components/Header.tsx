...
import { Resolution, AIModel, MODEL_DISPLAY_NAMES, ModelSource } from '../types';
...
export const Header: React.FC<HeaderProps> = ({ resolution, model, modelSource, localModelId }) => {
  return (
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <span className="text-[10px] bg-zinc-800 text-zinc-400 px-1.5 py-0.5 rounded font-mono border border-white/5 uppercase">{resolution} Mode</span>
              <p className="text-[10px] text-zinc-500 font-medium flex items-center gap-1">
                {modelSource === ModelSource.GEMINI ? MODEL_DISPLAY_NAMES[model] : (localModelId || 'Local Model')} <Sparkles className="w-2.5 h-2.5 text-cyan-400" />
              </p>
            </div>
          </div>
        </div>
